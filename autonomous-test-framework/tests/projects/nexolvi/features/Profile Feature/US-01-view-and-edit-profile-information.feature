Feature: Profile Feature — US-01 View and Edit Profile Information
  ...
  @positive @ac-1 @ac-2 @ac-3 @smoke @functional @tc-001 @held
  ...
  Scenario: [TC-002] Valid credentials redirect to Dashboard and profile button displays character B
    ...
    When the user enters "bikash.htc@gmail.com" in the Email field and "admin@123" in the Password field and clicks Sign In
    Then the user is redirected to "https://admin.nexolvi.ai/" and the page displays the header 'Dashboard'
  ...
  @positive @ac-13 @ac-16 @ac-17 @br-2 @functional @tc-004 @held
  Scenario: [TC-004] Saving profile with blank password fields preserves existing password and reflects updated info
    ...
    Then the profile changes are saved and the existing password is preserved unchanged
    When the user closes and reopens the My Profile modal
    Then the Name field displays {{updatedName}} reflecting the updated profile information

  @positive @ac-15 @ac-16 @br-3 @functional @tc-005 @held
  Scenario: [TC-005] Saving profile with matching non-empty passwords accepts the new password
    ...
    Then password validation passes and the updated profile information is saved successfully
  ...
  Scenario: [TC-006] Mismatched New Password and Confirm Password values reject the save with a mismatch error
    ...
    Then the save is rejected and a password mismatch error is indicated before any profile changes are persisted with the text "Error: with the text "Error
  ...
  Scenario: [TC-007] Sign In button remains disabled when only the Email field contains a value
    ...
    When the user enters "bikash.htc@gmail.com" in the Email field and leaves the Password field empty
    Then the Sign In button is disabled

  @negative @ac-2 @br-1 @functional @tc-008 @held
  Scenario: [TC-008] Sign In button remains disabled when only the Password field contains a value
    Given the user is on the login page at "https://admin.nexolvi.ai/login"
    Then the Email field and Password field are displayed and both are empty
    When the user leaves the Email field empty and enters "admin@123" in the Password field
    Then the Sign In button is disabled
  ...
  Scenario: [TC-009] Clicking Cancel discards unsaved changes and closes the modal leaving the user on the Dashboard
    Given the user has the My Profile modal open with unsaved changes entered in the Name field
    Then the modal is displayed with the unsaved changes visible
    When the user clicks 'Cancel'
    Then the My Profile modal is closed, the unsaved changes are discarded, and the Dashboard is displayed

  @edge @ac-3 @br-1 @functional @tc-010 @held
  ...
  Scenario: [TC-012] My Profile modal and user menu display exact UI text and profile button shows correct tooltip
    ...
    Then a native browser tooltip displaying 'bikash.h' is shown
    When the user clicks the profile button
    Then the user menu displays the full name 'Bikash h', email 'bikash.htc@gmail.com', a 'Profile' button, and a 'Logout' button
  ...
  Scenario: [TC-013] New Password and Confirm Password fields mask entered characters
    ...
    Then the characters entered in the New Password field are masked
    And the characters entered in the Confirm Password field are masked
